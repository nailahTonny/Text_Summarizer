...
if uploaded_file:
    ...
    if sentiment_scores:
        ...
        sentiment_labels = [sentiment_label(p) for p in sentiment_scores]
        sentiment_colors = {
            "Positive": "#81C784",
            "Neutral": "#FFEB3B",
            "Negative": "#E57373"
        ...
        fig, ax = plt.subplots(figsize=(8, 2.5))  # smaller figure

        for i, label in enumerate(sentiment_labels):
            ax.bar(i, 1, color=sentiment_colors[label])

        ax.set_yticks([])
        ax.set_xticks([])
        ax.set_xlim(0, len(sentiment_labels))
        ax.set_title("Sentiment Timeline (Positive / Neutral / Negative)", fontsize=13, color="#333")
        st.pyplot(fig)
